...
import quantumstudios.culinae.api.process.Boiling;
...
import quantumstudios.culinae.tiles.FuelHeatHandler;
import quantumstudios.culinae.tiles.FuelHeatHandler.FuelInfo;
...
public class JEICompat implements IModPlugin
{
    ...
    static final ResourceLocation CULINAE_RECIPE_GUI = new ResourceLocation(culinae.MODID, "textures/gui/jei.png");
    ...
    public void register(IModRegistry registry)
    {
        ...
        {
            updateAxesList(CulinaeConfig.CLIENT.axeList);
        }

        IGuiHelper guiHelper = registry.getJeiHelpers().getGuiHelper();
        arrowOut = guiHelper.createDrawable(JEICompat.CULINAE_RECIPE_GUI, 11, 26, 11, 7);
        arrowOutOverlay = guiHelper.drawableBuilder(JEICompat.CULINAE_RECIPE_GUI, 11, 18, 11, 8).buildAnimated(new CombinedTimer(80, 11, 11, 22), IDrawableAnimated.StartDirection.LEFT);
        arrowIn = guiHelper.createDrawable(JEICompat.CULINAE_RECIPE_GUI, 0, 26, 11, 7);
        IDrawableBuilder builder = guiHelper.drawableBuilder(JEICompat.CULINAE_RECIPE_GUI, 0, 18, 11, 8);
        ...
        registry.getJeiHelpers().getIngredientBlacklist().addIngredientToBlacklist(new ItemStack(CulinaeRegistry.INGREDIENT));
        registry.getJeiHelpers().getIngredientBlacklist().addIngredientToBlacklist(new ItemStack(CulinaeRegistry.DRINK));
        registry.getJeiHelpers().getIngredientBlacklist().addIngredientToBlacklist(new ItemStack(CulinaeRegistry.BOTTLE));
        registry.getJeiHelpers().getIngredientBlacklist().addIngredientToBlacklist(CulinaeRegistry.BASIC_FOOD.getItemStack(ItemBasicFood.Variant.EMPOWERED_CITRON));

        // Don't shove things into JEI when the recipe is not here.
        if (ForgeRegistries.RECIPES.containsKey(new ResourceLocation(culinae.MODID, "chopping_board")))
        {
        ...
        registry.addRecipes(CulinaeInternalGateway.INSTANCE.itemToSpiceMapping.keySet().stream().map(RecipeSpiceBottleFillingWrapper::new).collect(Collectors.toList()), VanillaRecipeCategoryUid.CRAFTING);
        registry.addRecipes(CulinaeInternalGateway.INSTANCE.oreDictToSpiceMapping.keySet().stream().map(OreDictDefinition::of).map(RecipeSpiceBottleFillingWrapper::new).collect(Collectors.toList()), VanillaRecipeCategoryUid.CRAFTING);

        BlockChoppingBoard.getSuitableCovers().stream().map(CulinaeRegistry.CHOPPING_BOARD::getItemStack).forEach(stack -> registry.addRecipeCatalyst(stack, ChoppingBoardRecipeCategory.UID));
        ...
        for (Map.Entry<ItemDefinition, Ingredient> entry : CulinaeInternalGateway.INSTANCE.itemIngredients.entrySet())
        {
        ...
        for (Map.Entry<String, Ingredient> entry : CulinaeInternalGateway.INSTANCE.oreDictIngredients.entrySet())
        {
            if (entry.getValue().getForm() != Form.JUICE)
            {
        ...
        CulinaeInternalGateway.INSTANCE.itemIngredients.forEach((k, v) -> {
        ...
        CulinaeInternalGateway.INSTANCE.oreDictIngredients.forEach((k, v) -> {
            if (v.getForm() == Form.FULL && !v.getMaterial().getValidForms().isEmpty())
            {
        ...
        recipes.clear();
        registry.addRecipeCatalyst(CulinaeRegistry.ITEM_MORTAR.getItemStack(ItemMortar.Variant.EMPTY), MortarRecipeCategory.UID);
        ...
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.MILL), MillRecipeCategory.UID);
        ...
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.JAR), VesselRecipeCategory.UID);
        ...
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.WOODEN_BASIN), BasinSqueezingRecipeCategory.UID, BasinThrowingRecipeCategory.UID);
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.EARTHEN_BASIN), BasinSqueezingRecipeCategory.UID, BasinThrowingRecipeCategory.UID, BoilingRecipeCategory.UID);
        for (int i = 0; i < CulinaeRegistry.EARTHEN_BASIN_COLORED.getItemSubtypeAmount(); i++)
        {
            registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.EARTHEN_BASIN_COLORED, 1, i), BasinSqueezingRecipeCategory.UID, BasinThrowingRecipeCategory.UID, BoilingRecipeCategory.UID);
        }
        ...
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 1), FirepitFuelCategory.UID);
        registry.addRecipeCatalyst(new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 2), FirepitFuelCategory.UID);
    ...
    {
        IGuiHelper guiHelper = registry.getJeiHelpers().getGuiHelper();
        IDrawable basin = guiHelper.createDrawable(JEICompat.CULINAE_RECIPE_GUI, 0, 33, 20, 10);
