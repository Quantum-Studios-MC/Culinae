...
public class CulinaePlantHandler implements IPlantHandler
{
    private final Variant variant;

    public CulinaePlantHandler(Variant variant)
    {
    ...
    {
        return seed.getItem() == CulinaeRegistry.CROPS && seed.getMetadata() == variant.getMeta();
    }
    ...
    public ItemStack[] getOutput(ItemStack seed, ItemStack soil, TileEntity tile)
    {
        ...
        if (variant == Variant.BAMBOO_SHOOT) // cannot tint?
        {
            output = new ItemStack(CulinaeRegistry.BAMBOO, 2);
        }
        else
        {
