...
public class PatchouliClientHandler implements IResourceManagerReloadListener
{
    ...
    {
        ClientBookRegistry.INSTANCE.pageTypes.put("culinae:centered_text", PageCenteredText.class);
        ClientBookRegistry.INSTANCE.pageTypes.put("culinae:drink_type", PageDrinkType.class);
    ...
    private static void registerMacro(FunctionProcessor function, String... names)
    {
        ...
        {
            // culinae.logger is not initialized at this point. We have to use another one.
            Kiwi.logger.debug("Culinae failed to inject 3rd party marcos into Patchouli, exception:\n {}", e);
        }
    }
    ...
    public void reload()
    {
        Book book = BookRegistry.INSTANCE.books.get(new ResourceLocation(culinae.MODID, "culinary_101"));
        if (book != null)
        {
            registerSubtypesMapping(book, CulinaeRegistry.CROPS, "preface/plant");
            registerMapping(book, new ItemStack(CulinaeRegistry.BAMBOO), "preface/plant");
            registerMapping(book, new ItemStack(Blocks.FARMLAND), "preface/plant");

            registerMapping(book, new ItemStack(CulinaeRegistry.WOODEN_BASIN), "processing/basin");
            registerMapping(book, new ItemStack(CulinaeRegistry.EARTHEN_BASIN), "processing/basin");
            registerSubtypesMapping(book, CulinaeRegistry.EARTHEN_BASIN_COLORED, "processing/basin");
            registerMapping(book, new ItemStack(CulinaeRegistry.ITEM_MORTAR), "processing/mortar");
            registerMapping(book, new ItemStack(CulinaeRegistry.JAR), "processing/jar");
            registerMapping(book, new ItemStack(CulinaeRegistry.MILL), "processing/mill");
            registerMapping(book, new ItemStack(CulinaeRegistry.CHOPPING_BOARD), "processing/chopping_board");

            registerMapping(book, new ItemStack(Items.BREWING_STAND), "utensils/brewing");
            registerMapping(book, new ItemStack(CulinaeRegistry.DISH), "utensils/plate");
            registerMapping(book, new ItemStack(CulinaeRegistry.DRINKRO), "utensils/drinkro");
            registerMapping(book, new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 0), "utensils/fire_pit");
            registerMapping(book, new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 1), "utensils/wok");
            registerMapping(book, new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 2), "utensils/bbq_rack");
            registerMapping(book, new ItemStack(CulinaeRegistry.FIRE_PIT, 1, 3), "utensils/frying_pan");

            registerSubtypesMapping(book, CulinaeRegistry.SHEARED_LEAVES, "misc/fruit_trees");
            registerSubtypesMapping(book, CulinaeRegistry.SAPLING, "misc/fruit_trees");
            registerMapping(book, new ItemStack(CulinaeRegistry.LOG), "misc/fruit_trees");
            registerMapping(book, new ItemStack(CulinaeRegistry.TOFU_BLOCK), "misc/tofu_block");
    ...
    {
        BookEntry bookEntry = book.contents.entries.get(new ResourceLocation(culinae.MODID, entry));
        if (bookEntry != null)
        {
