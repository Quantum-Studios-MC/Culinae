...
public class CTMill
{
    ...
    private static CulinaeProcessingRecipeManager<Milling> getManager()
    {
    ...
    private static final class Addition extends CTSupport.Addition
    {
        ...
        {
            return String.format("Add Culinae Mill recipe: %s + %s -> %s + %s", actualInput, actualInputFluid, actualOutput, actualOutputFluid);
        }
    }
    ...
    private static final class ItemBasedRemoval implements IAction
    {
        ...
        public String describe()
        {
            return String.format("Remove all Culinae milling recipes that has input item of %s and input fluid of %s", actualInput, actualInputFluid);
        }
    }
