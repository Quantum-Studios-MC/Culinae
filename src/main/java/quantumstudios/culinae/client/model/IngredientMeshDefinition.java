...
public final class IngredientMeshDefinition implements ItemMeshDefinition, ISelectiveResourceReloadListener
{
    ...
    public ModelResourceLocation getModelLocation(ItemStack stack)
    {
        if (stack.getItem() == CulinaeRegistry.INGREDIENT)
        {
            ...
            if (data != null)
            {
                String formName = data.getString(CulinaeSharedSecrets.KEY_FORM);
                Form form = Form.of(formName);
                if (form != null)
                {
                    String material = data.getString(CulinaeSharedSecrets.KEY_MATERIAL);
                    return new ModelResourceLocation(new ResourceLocation(culinae.MODID, overrides.get(form).getOrDefault(material, "cmaterial/" + formName.toLowerCase(Locale.ENGLISH))), "inventory");
                }
            }
        }
        return new ModelResourceLocation(CulinaeItemRendering.EMPTY_MODEL, "inventory"); // Catch all
    }
    ...
    public void onResourceManagerReload(IResourceManager manager, Predicate<IResourceType> tester)
    {
        if (tester.test(VanillaResourceType.MODELS))
        {
            ...
            /*overrides.values()
                    ...
                    .map(path -> new ResourceLocation(culinae.MODID, path))
                    .forEach(path -> ModelLoader.registerItemVariants(CulinaeRegistry.INGREDIENT, path));*/
        }
    }
}
