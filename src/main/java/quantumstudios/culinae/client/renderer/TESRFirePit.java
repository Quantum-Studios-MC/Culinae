...
import quantumstudios.culinae.client.renderer.HoloProfiles.HoloProfile;
...
public abstract class TESRFirePit<T extends TileFirePit> extends TileEntitySpecialRenderer<T>
{
    ...
    public void render(T tile, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
        ...
        if (CulinaeConfig.CLIENT.showHoloGui && !mc.gameSettings.hideGUI)
        {
            ...
            if (transparency > 0)
            {
                ...
                mc.getTextureManager().bindTexture(CulinaeGUI.TEXTURE_ICONS);
