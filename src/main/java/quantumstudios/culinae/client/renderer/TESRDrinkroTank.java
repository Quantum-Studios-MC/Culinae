...
import quantumstudios.culinae.tiles.TileDrinkroTank;
...
public class TESRDrinkroTank extends TileEntitySpecialRenderer<TileDrinkroTank>
{
    ...
    public void render(TileDrinkroTank te, double x, double y, double z, float partialTicks, int destroyStage, float alpha)
    {
        ...
        if (meta == 0)
        {
            if (!CulinaeConfig.CLIENT.alwaysRenderDrinkro && z > 0)
            {
                return;
            }
            rot = 180;
        ...
        {
            if (!CulinaeConfig.CLIENT.alwaysRenderDrinkro && x < -1)
            {
                return;
            }
            rot = 90;
        ...
        {
            if (!CulinaeConfig.CLIENT.alwaysRenderDrinkro && z < -1)
            {
                return;
            }
        }
        ...
        {
            if (!CulinaeConfig.CLIENT.alwaysRenderDrinkro && x > 0)
            {
                return;
            }
            rot = -90;
        ...
        if (size > 0)
        {
            ...
            TextureAtlasSprite flowing = mc.getTextureMapBlocks().getTextureExtry(CulinaeFluids.JUICE.getFlowing().toString());
            if (flowing == null)
            {
            ...
            {
                TextureAtlasSprite still = mc.getTextureMapBlocks().getTextureExtry(CulinaeFluids.JUICE.getStill().toString());
                if (still == null)
                {
