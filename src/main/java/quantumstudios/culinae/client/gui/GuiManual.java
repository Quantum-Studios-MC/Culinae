package quantumstudios.culinae.client.gui;
...
{
    private static final ResourceLocation BOOK_GUI_TEXTURES = new ResourceLocation(culinae.MODID, "textures/gui/patchouli.png");
    ...
    public void confirmClicked(boolean result, int id)
    {
        if (result && (id == 3 || id == 4))
        {
            ...
            {
                culinae.logger.error("The chat room link '{}' seems to be malformed", chatRoomURL);
                culinae.logger.debug("Exception caught: {}", wrongURI);
            }
            catch (Exception e)
            {
                culinae.logger.error("Couldn't open link", e);
            }
        }
        mc.displayGuiScreen(null);
    }
