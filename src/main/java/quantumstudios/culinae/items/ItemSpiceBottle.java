...
public class ItemSpiceBottle extends ItemMod
{
    ...
    public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> items)
    {
        if (this.isInCreativeTab(tab))
        {
            ...
            CulinaeInternalGateway.INSTANCE.fluidToSpiceMapping.keySet().forEach(f -> {
            ...
            CulinaeInternalGateway.INSTANCE.itemToSpiceMapping.keySet().forEach(id -> {
                ItemStack copy = stack.copy();
                getItemHandler(copy).insertItem(0, id.getItemStack(), false);
                items.add(copy);
            });
            CulinaeInternalGateway.INSTANCE.oreDictToSpiceMapping.keySet().forEach(ore -> {
                ItemStack copy = stack.copy();
