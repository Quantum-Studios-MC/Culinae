...
import quantumstudios.culinae.items.ItemMortar.Variant;
...
public class ItemMortar extends ItemModVariantsNew<Void, Variant>
{
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        if (meta != Variant.WATER.getMeta())
        {
            BlockPos result = PlayerUtil.tryPlaceBlock(worldIn, pos, side, player, hand, CulinaeRegistry.MORTAR.getStateForPlacement(worldIn, pos, side, hitX, hitY, hitZ, 0, player, hand), stack, false);
