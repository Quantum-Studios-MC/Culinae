...
public interface CulinarySkill
{
    ...
    static void register(CulinarySkill type)
    {
        try
        {
            Class.forName("quantumstudios.culinae.internal.CulinarySkillManager").getMethod("register", CulinarySkill.class).invoke(null, type);
        }
        catch (Throwable e)
        {
            e.printStackTrace();
    ...
    static int getPlayerSkillLevel(EntityPlayerMP player, CulinarySkill type)
    {
        try
        {
            return (int) Class.forName("quantumstudios.culinae.util.CulinarySkillUtil").getMethod("getPlayerSkillLevel", EntityPlayerMP.class, String.class).invoke(null, player, type.getName());
        }
        catch (Throwable e)
        {
            e.printStackTrace();
