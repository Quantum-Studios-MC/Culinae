...
import quantumstudios.culinae.client.model.DrinkroMeshDefinition;
import quantumstudios.culinae.tiles.TileDrinkroBase;
...
public class BlockDrinkro extends BlockModHorizontal
{
    ...
    public void mapModel()
    {
        ...
        ModelBakery.registerItemVariants(item, new ResourceLocation(culinae.MODID, "drinkro"), new ResourceLocation(culinae.MODID, "drinkro_special"));
    }
        ...
        if (inv != null)
        {
            if (held.isEmpty() || ItemHandlerHelper.insertItem(inv, held, true).getCount() == held.getCount())
            {
                for (int i = inv.getSlots() - 1; i >= 0; i--)
                {
                    ...
                    {
                        playerIn.openGui(culinae.getInstance(), CulinaeGUI.NAME_FOOD, worldIn, pos.getX(), pos.getY() + 1, pos.getZ());
                        break;
                    }
                    if (!stack.isEmpty())
                    {
