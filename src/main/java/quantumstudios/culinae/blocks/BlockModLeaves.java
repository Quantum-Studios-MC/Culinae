...
import quantumstudios.culinae.blocks.BlockModSapling.Type;
import quantumstudios.culinae.items.ItemBasicFood;
import quantumstudios.culinae.library.RarityManager;
import quantumstudios.culinae.tiles.TileFruitTree;
...
public class BlockModLeaves extends BlockMod implements IGrowable, IShearable
{
    ...
    static
    {
        if (Loader.isModLoaded("passableleaves") || CulinaeConfig.GENERAL.passableLeaves)
        {
            passable = true;
        }
    }
    ...
    {
        ItemStack drop = CulinaeRegistry.BASIC_FOOD.getItemStack(fruit);
    ...
    {
        if (!(CulinaeConfig.GENERAL.fruitDrops && worldIn.getGameRules().getBoolean("doTileDrops")) && state.getValue(AGE) == 3)
        {
            return false;
        }
    ...
    {
        return CulinaeRegistry.SHEARED_LEAVES.getItemInternal(getShearedState(state));
    }

    @Override
    public void getDrops(NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune)
    {
        CulinaeRegistry.SHEARED_LEAVES.getDrops(drops, world, pos, getShearedState(state), fortune);
    }
    ...
    {
        IBlockState newState = CulinaeRegistry.SHEARED_LEAVES.getDefaultState();
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
    {
        if (!worldIn.isRemote)
        {
            ...
            {
                boolean def = rand.nextInt(100) > 99 - CulinaeConfig.GENERAL.fruitGrowingSpeed;
    ...
    {
        if (!worldIn.isRemote && state.getBlock() == CulinaeRegistry.LEAVES_CITRON && blockIn == Blocks.AIR && fromPos.equals(pos.up()) && worldIn.getBlockState(fromPos).getBlock() == Blocks.FIRE)
        {
