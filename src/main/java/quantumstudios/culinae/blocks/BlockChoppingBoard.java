...
import quantumstudios.culinae.library.UnlistedPropertyItemStack;
import quantumstudios.culinae.network.PacketCustomEvent;
import quantumstudios.culinae.tiles.TileChoppingBoard;
import quantumstudios.culinae.tiles.TileChoppingBoard.ProcessionType;
...
public class BlockChoppingBoard extends BlockMod
{
    ...
    public void onBlockClicked(World worldIn, BlockPos pos, EntityPlayer playerIn)
    {
        ...
        if (te instanceof TileChoppingBoard)
        {
            ...
            else if (!worldIn.isRemote && CulinaeConfig.GENERAL.axeChopping && playerIn.getCooledAttackStrength(0) > 0.5)
            {
    ...
    public static void onChoppingBoardClick(PlayerInteractEvent.LeftClickBlock event)
    {
        ...
        if (event.getEntityPlayer().isCreative() && state.getBlock() == CulinaeRegistry.CHOPPING_BOARD)
        {
