...
import quantumstudios.culinae.tiles.TileBasin;
...
public class BlockBasin extends BlockMod
{
    ...
    public void onFallenUpon(World worldIn, BlockPos pos, Entity entityIn, float fallDistance)
    {
        ...
        if (tile instanceof TileBasin)
        {
            ...
            if (fallDistance >= 1 && entityIn instanceof EntityLivingBase)
            {
                ...
                else if (input.getItem() == CulinaeRegistry.BASIC_FOOD && input.getMetadata() == ItemBasicFood.Variant.EMPOWERED_CITRON.getMeta() && entityIn instanceof EntityPlayer && tileBasin.tank.getFluidAmount() == 0)
                {
    ...
    {
        if (CulinaeConfig.GENERAL.enableSqueezer && pos.up().equals(fromPos))
        {
            IBlockState fromState = worldIn.getBlockState(fromPos);
            if (fromState.getBlock() == Blocks.PISTON)
            {
                worldIn.setBlockState(fromPos, CulinaeRegistry.SQUEEZER.getDefaultState());
            }
        }
