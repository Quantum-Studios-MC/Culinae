...
import quantumstudios.culinae.tiles.TileBarbecueRack;
...
public class BlockFirePit extends BlockModHorizontal
{
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
        ...
        if (stack.getItem() == CulinaeRegistry.WOK && hasComponent(state, Component.NONE))
        {
        ...
        if (stack.getItem() == CulinaeRegistry.FAN)
        {
            return false;
        }
    ...
    {
        drops.add(new ItemStack(CulinaeRegistry.FIRE_PIT));
        if (hasComponent(state, Component.WOK))
        {
            drops.add(new ItemStack(CulinaeRegistry.WOK));
        }
