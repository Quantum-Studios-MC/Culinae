...
public class DrinkBrewingRecipe implements IBrewingRecipe
{
    ...
    {
        if (input.getItem() != CulinaeRegistry.BOTTLE)
        {
            return false;
        }
        // Why not "Potion":
        ...
        if (input.hasCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null))
        {
            ...
            {
                return BREWABLE_MATERIALS.contains(fluid.tag.getString(CulinaeSharedSecrets.KEY_MATERIAL));
            }
        }
        return false;
    }
    ...
    {
        if (stack.getItem() == CulinaeRegistry.BOTTLE)
        {
