...
public class RecipeSpiceBottleFilling extends AbstractDynamicShapedRecipe
{
    ...
    protected boolean checkMatch(InventoryCrafting inv, int startX, int startY)
    {
        ...
        if (bottle.getItem() != CulinaeRegistry.SPICE_BOTTLE || !CulinaeRegistry.SPICE_BOTTLE.isContainerEmpty(bottle))
        {
            return false;
        }
    ...
    public ItemStack getCraftingResult(InventoryCrafting inv)
    {
        ...
        for (int k = 0; k < inv.getSizeInventory(); ++k)
        {
            ...
            if (!stack.isEmpty())
            {
                if (stack.getItem() == CulinaeRegistry.SPICE_BOTTLE)
                {
                    bottle = stack.copy();
                    bottle.setCount(1);
                }
        ...
        CulinaeRegistry.SPICE_BOTTLE.getItemHandler(bottle).insertItem(0, spice, false);
        return bottle;
    }
