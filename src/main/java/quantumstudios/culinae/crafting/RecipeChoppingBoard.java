    ...
    public ItemStack getCraftingResult(InventoryCrafting inv)
    {
        for (int x = 0; x <= inv.getWidth() - getRecipeWidth(); ++x)
        {
            for (int y = 0; y <= inv.getHeight() - getRecipeHeight(); ++y)
            {
                ...
                {
                    return CulinaeRegistry.CHOPPING_BOARD.getItemStack(inv.getStackInRowAndColumn(x, y + 1));
                }
            }
        }
    ...
    protected boolean checkEmpty(InventoryCrafting inv, int startX, int startY)
    {
        for (int x = 0; x < inv.getWidth(); ++x)
        {
            for (int y = 0; y < inv.getHeight(); ++y)
            {
                ...
                if (!Ingredient.EMPTY.apply(stack) && stack.getItem() != CulinaeRegistry.KITCHEN_KNIFE)
                {
                    return false;
                }
            }
