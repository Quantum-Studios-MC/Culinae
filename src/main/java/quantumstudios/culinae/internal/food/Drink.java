...
import quantumstudios.culinae.api.prefab.DefaultConsumedCollector;
...
public class Drink extends CompositeFood
{
    ...
    public static class DrinkType
    {
        ...
        public static final DrinkType SMOOTHIE = new DrinkType("smoothie", ItemDefinition.of(CulinaeRegistry.PLACED_DISH), CulinaeRegistry.TOUGHNESS, MobEffects.RESISTANCE);
        public static final DrinkType GELO = new DrinkType("gelo", ItemDefinition.EMPTY, MobEffects.JUMP_BOOST, MobEffects.SPEED);
        public static final DrinkType SODA = new DrinkType("soda", ItemDefinition.of(Items.GLASS_BOTTLE), CulinaeRegistry.COLD_BLOOD, MobEffects.STRENGTH);
        ...
        public String getTranslationKey()
        {
            if (this == SMOOTHIE)
            {
                ...
                {
                    return culinae.MODID + ".snownee";
                }
            }
            return culinae.MODID + "." + name;
        }
    ...
    public static final ResourceLocation DRINK_ID = new ResourceLocation(culinae.MODID, "drink");
    ...
    {
        return new ItemStack(CulinaeRegistry.DRINK);
    }
    ...
    {
        return Kiwi.isLoaded(new ResourceLocation(culinae.MODID, "toughasnails")) && TANCompat.enableThirst();
    }
    ...
    public void onEaten(ItemStack stack, World worldIn, EntityPlayer player)
    {
        ...
        for (Seasoning seasoning : seasonings)
        {
            ...
            else if (CulinaeConfig.HARDCORE.enable && CulinaeConfig.HARDCORE.badSkillPunishment && (seasoning.getSpice() == CulinaryHub.CommonSpices.SOY_SAUCE || seasoning.getSpice() == CulinaryHub.CommonSpices.CHILI_POWDER || seasoning.getSpice() == CulinaryHub.CommonSpices.SICHUAN_PEPPER_POWDER))
            {
                modifier = 0;
                break;
            }
        ...
        if (!worldIn.isRemote)
        {
            ...
            PotionEffect effect = player.getActivePotionEffect(CulinaeRegistry.EFFECT_RESISTANCE);
            if (effect != null)
            {
                player.removePotionEffect(CulinaeRegistry.EFFECT_RESISTANCE);
                player.addPotionEffect(new PotionEffect(CulinaeRegistry.EFFECT_RESISTANCE, (int) (effect.getDuration() * 0.25), effect.getAmplifier(), effect.getIsAmbient(), effect.doesShowParticles()));
            }
        }
    ...
    public static NBTTagCompound serialize(Drink drink)
    {
        ...
        data.setInteger(CulinaeSharedSecrets.KEY_FOOD_LEVEL, drink.getFoodLevel());
        data.setFloat(CulinaeSharedSecrets.KEY_SATURATION_MODIFIER, drink.getSaturationModifier());
        data.setInteger(CulinaeSharedSecrets.KEY_SERVES, drink.getServes());
        data.setFloat(CulinaeSharedSecrets.KEY_USE_DURATION, drink.getUseDurationModifier());
        return data;
    }
