...
import quantumstudios.culinae.api.*;
import quantumstudios.culinae.api.prefab.MaterialBuilder;
...
import quantumstudios.culinae.internal.effect.*;
import quantumstudios.culinae.internal.food.Dish;
import quantumstudios.culinae.internal.food.Drink;
import quantumstudios.culinae.internal.material.*;
import quantumstudios.culinae.internal.spice.SpiceChiliPowder;
...
/**
 * The main implementation of CulinaeAPI.
 *
 * CulinaeAPI 的主要实现。
 */
public final class CulinaeInternalGateway implements CulinaeAPI
{

    /**
     * The singleton reference of CulinaeInternalGateway, for internal usage.
     *
     * CulinaeInternalGateway 的单例，供内部使用。
     */
    public static CulinaeInternalGateway INSTANCE;
    ...
    private CulinaeInternalGateway()
    {
    ...
    public <F extends CompositeFood> NBTTagCompound serialize(F dishObject)
    {
        ...
        else
        {
            ...
            data.setString(CulinaeSharedSecrets.KEY_TYPE, identifier);
            return data;
        }
    }
    ...
    {
        ResourceLocation rl = new ResourceLocation(data.getString(CulinaeSharedSecrets.KEY_TYPE));
        if (!identifier.equals(rl))
        {
            culinae.logger.warn("Cannot use {} to deserialize CompositeFood with type of {}", identifier, rl);
            return null;
        }
    ...
    public Ingredient findIngredient(ItemStack item)
    {
        ...
        if (item.getItem() == CulinaeRegistry.INGREDIENT)
        {
            ...
            {
                return CulinaePersistenceCenter.deserializeIngredient(data);
            }
        }
    ...
    public Ingredient findIngredient(@Nullable FluidStack fluid)
    {
        ...
        if (fluid.getFluid() == CulinaeFluids.JUICE) // Special-casing Culinae "juice" fluid
        {
            if (fluid.tag == null || !fluid.tag.hasKey(CulinaeSharedSecrets.KEY_MATERIAL, Constants.NBT.TAG_STRING))
            {
                return null;
            }
            Material material = findMaterial(fluid.tag.getString(CulinaeSharedSecrets.KEY_MATERIAL));
    ...
    /**
     * Internal hook for initialization of CulinaeAPI instance. Not intended for public usage.
    ...
    {
        // API initialization
        CulinaeInternalGateway api = new CulinaeInternalGateway();
        // Distribute API references
        CulinaryHub.API_INSTANCE = CulinaeInternalGateway.INSTANCE = api;
        ...
        api.register(new EffectPotions("hot").addPotionEffect(new PotionEffect(CulinaeRegistry.HOT, 1200, 0)));
        api.register(new EffectPotions("dispersal").addPotionEffect(new PotionEffect(CulinaeRegistry.DISPERSAL, 400, 1)));
    ...
    {
        CulinaeInternalGateway api = CulinaeInternalGateway.INSTANCE;
        api.itemIngredients.put(ItemDefinition.of(CulinaeRegistry.CROPS, ItemCrops.Variant.RED_PEPPER.getMeta()), new Ingredient(CulinaryHub.CommonMaterials.RED_PEPPER, Form.FULL));
        api.itemIngredients.put(ItemDefinition.of(CulinaeRegistry.BASIC_FOOD, ItemBasicFood.Variant.EMPOWERED_CITRON.getMeta()), new Ingredient(CulinaryHub.CommonMaterials.EMPOWERED_CITRON, Form.FULL));
        ...
        api.fluidIngredients.put(CulinaeFluids.MILK.getName(), new Ingredient(CulinaryHub.CommonMaterials.MILK, Form.JUICE));
        api.fluidIngredients.put(CulinaeFluids.SOY_MILK.getName(), new Ingredient(CulinaryHub.CommonMaterials.SOY_MILK, Form.JUICE));

        api.fluidToSpiceMapping.put(CulinaeFluids.EDIBLE_OIL.getName(), CulinaryHub.CommonSpices.EDIBLE_OIL);
        api.fluidToSpiceMapping.put(CulinaeFluids.SESAME_OIL.getName(), CulinaryHub.CommonSpices.SESAME_OIL);
        api.fluidToSpiceMapping.put(CulinaeFluids.SOY_SAUCE.getName(), CulinaryHub.CommonSpices.SOY_SAUCE);
        api.fluidToSpiceMapping.put(CulinaeFluids.RICE_VINEGAR.getName(), CulinaryHub.CommonSpices.RICE_VINEGAR);
        api.fluidToSpiceMapping.put(CulinaeFluids.FRUIT_VINEGAR.getName(), CulinaryHub.CommonSpices.FRUIT_VINEGAR);
        api.fluidToSpiceMapping.put(FluidRegistry.WATER.getName(), CulinaryHub.CommonSpices.WATER);

        api.itemToSpiceMapping.put(ItemDefinition.of(CulinaeRegistry.MATERIAL, culinae.Materials.CHILI_POWDER.getMeta()), CulinaryHub.CommonSpices.CHILI_POWDER);
        api.itemToSpiceMapping.put(ItemDefinition.of(CulinaeRegistry.MATERIAL, culinae.Materials.SICHUAN_PEPPER_POWDER.getMeta()), CulinaryHub.CommonSpices.SICHUAN_PEPPER_POWDER);
        api.itemToSpiceMapping.put(ItemDefinition.of(Items.SUGAR), CulinaryHub.CommonSpices.SUGAR);
        api.itemToSpiceMapping.put(ItemDefinition.of(CulinaeRegistry.MATERIAL, culinae.Materials.UNREFINED_SUGAR.getMeta()), CulinaryHub.CommonSpices.UNREFINED_SUGAR);
    ...
    //        CulinaeAPI.super.registerMapping(item, material);
    //    }
    ...
    //    {
    //        // TODO Auto-generated method stub
    //        CulinaeAPI.super.registerMapping(ore, material);
    //    }
    ...
    {
        return CulinaeRegistry.EFFECT_RESISTANCE;
    }
