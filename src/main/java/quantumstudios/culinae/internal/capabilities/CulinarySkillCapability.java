...
import quantumstudios.culinae.api.CulinarySkillPointContainer;
import quantumstudios.culinae.api.prefab.SimpleCulinarySkillPointContainerImpl;
    ...
    static final class EventListener
    {
        ...
        public static void onPlayerGatherCapabilities(AttachCapabilitiesEvent<Entity> event)
        {
            if (event.getObject() instanceof EntityPlayer)
            {
                ...
                {
                    event.addCapability(new ResourceLocation(culinae.MODID, "culinary_skill"), new DefaultProvider());
                }
            }
        }
            ...
            if (skill != null)
            {
                ...
                if (original != null)
                {
                    for (CulinarySkillPoint p : original.getAvailableSkillPoints())
                    {
                        ...
                        if (event.isWasDeath() && CulinaeConfig.HARDCORE.enable && CulinaeConfig.HARDCORE.loseSkillPointsOnDeath)
                        {
                            syncedPoints *= CulinaeConfig.HARDCORE.skillPointsRetainRatio;
                        }
