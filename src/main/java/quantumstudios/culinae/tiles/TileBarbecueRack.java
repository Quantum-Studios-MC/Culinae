    ...
    public TileBarbecueRack()
    {
        stacks = new ItemStackHandler(4)
        {
            ...
            public boolean isItemValid(int slot, ItemStack stack)
            {
                if (slot < 3)
                {
                    return stack.getItem() == CulinaeRegistry.INGREDIENT || FurnaceRecipes.instance().getSmeltingResult(stack).getItem() instanceof ItemFood;
                }
                else
                {
        ...
        if (heatHandler.getHeatPower() > 0)
        {
            for (int i = 0; i < 3; ++i)
            {
                ...
                if (stack.getItem() == CulinaeRegistry.INGREDIENT)
                {
                    ...
                    double progress;
                    Material material = CulinaryHub.API_INSTANCE.findMaterial(helper.getString(CulinaeSharedSecrets.KEY_MATERIAL));
                    if (material == null)
                        progress = 1;
                    else
                    {
                    ...
                    {
                        burnTime[i] = 0;
                        int doneness = helper.getInt(CulinaeSharedSecrets.KEY_DONENESS);
                        helper.setInt(CulinaeSharedSecrets.KEY_DONENESS, doneness + (int) progress);
                    }
                }
                else
                {
