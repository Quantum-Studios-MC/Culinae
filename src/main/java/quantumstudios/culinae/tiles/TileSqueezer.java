...
public class TileSqueezer extends TileBase implements ITickable
{
     ...
     */
    private static final ResourceLocation STATE_MACHINE = new ResourceLocation(culinae.MODID, "asms/squeezer.json");
    ...
    public TileSqueezer()
    {
        this.stateMachine = culinae.sidedDelegate.loadAnimationStateMachine(STATE_MACHINE, ImmutableMap.of("offset", this.extensionOffset));
        if (CulinaeConfig.GENERAL.squeezerUsesFE > 0)
        {
            battery = new Battery(CulinaeConfig.GENERAL.squeezerUsesFE * 50, CulinaeConfig.GENERAL.squeezerUsesFE, 0)
            {
                @Override
    ...
    public void update()
    {
        ...
        else
        {
            if (triggered)
            {
                if ((state == State.EXTRACTED || state == State.EXTENDING) && battery.getEnergyStored() < CulinaeConfig.GENERAL.squeezerUsesFE)
                {
                    return;
                }
        ...
        if (this.state == State.EXTENDED && this.isInWorkCycle)
        {
            ...
            if (!world.isRemote)
            {
                ...
                if (tile instanceof TileBasin)
                {
                    ...
                    if (battery != null)
                    {
                        battery.setEnergy(battery.getEnergyStored() - CulinaeConfig.GENERAL.squeezerUsesFE);
                    }
                }
            }
        }
