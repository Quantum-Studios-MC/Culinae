...
import quantumstudios.culinae.library.FilterFluidHandler;
import quantumstudios.culinae.library.FilterItemHandler;
import quantumstudios.culinae.library.SingleSlotItemHandler;
...
public class TileMill extends TileBase implements ITickable
{
     ...
     */
    private static final ResourceLocation STATE_MACHINE_JSON = new ResourceLocation(culinae.MODID, "asms/mill.json");
    ...
    public TileMill()
    {
        stateMachine = culinae.sidedDelegate.loadAnimationStateMachine(STATE_MACHINE_JSON, ImmutableMap.of("progress", progressValue));
    ...
    public void onRightClicked(EntityPlayer playerIn)
    {
        ...
        if (power + CulinaeConfig.GENERAL.millWorkPeriod / 5 <= CulinaeConfig.GENERAL.millWorkPeriod)
        {
            power += CulinaeConfig.GENERAL.millWorkPeriod / 5;
    ...
    public void update()
    {
        ...
        if (!world.isRemote)
        {
            if (++tick >= CulinaeConfig.GENERAL.millWorkPeriod)
            {
                process();
                IBlockState state = this.world.getBlockState(this.pos);
        ...
        {
            if (++this.cycle >= CulinaeConfig.GENERAL.millWorkPeriod)
            {
                this.cycle = 0;
            }
            progressValue.setValue((float) this.cycle / CulinaeConfig.GENERAL.millWorkPeriod);
        }
    }
