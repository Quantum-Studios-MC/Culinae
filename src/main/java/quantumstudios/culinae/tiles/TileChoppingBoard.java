...
import quantumstudios.culinae.client.particle.ParticleQuad;
...
public class TileChoppingBoard extends TileInventoryBase
{
    ...
    public ItemStack insertItem(EntityPlayer player, ItemStack stack)
    {
        ...
        isAxe = CulinaeConfig.GENERAL.axeChopping && !con.isEmpty() && !OreUtil.doesItemHaveOreName(stack, "itemFoodCutter") && con.getItem() != CulinaeRegistry.INGREDIENT && !CulinaryHub.API_INSTANCE.isKnownIngredient(con) && (Processing.CHOPPING.findRecipe(con) != null);
        return ret;
    }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack)
    {
        ...
        if (CulinaeConfig.GENERAL.axeChopping && Processing.CHOPPING.findRecipe(stack) != null)
        {
            return true;
        }
        Ingredient ingredient = CulinaryHub.API_INSTANCE.findIngredient(stack);
    ...
    {
        return CulinaeRegistry.CHOPPING_BOARD.getItemStack(cover);
    }
    ...
    {
        if (stack.getItem() == CulinaeRegistry.KITCHEN_KNIFE)
        {
            return;
        }
    ...
    private static ItemStack craftMaterial(ItemStack raw, Ingredient ingredient, int[] actions, Random rand)
    {
        ...
        else if (raw.getItem() != CulinaeRegistry.INGREDIENT)
        {
