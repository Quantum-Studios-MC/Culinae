...
public class TileWok extends TileFirePit implements CookingVessel
{
    ...
    public void onActivated(EntityPlayerMP playerIn, EnumHand hand, EnumFacing facing)
    {
        ...
        case WORKING:
            ...
            else if (heldThing.getItem() == Item.getItemFromBlock(CulinaeRegistry.PLACED_DISH) && !builder.getIngredients().isEmpty())
            {
                ...
                playerIn.openGui(culinae.getInstance(), CulinaeGUI.NAME_FOOD, world, pos.getX(), pos.getY(), pos.getZ());
    ...
    private boolean cook(EntityPlayerMP player, EnumHand hand, ItemStack heldThing, EnumFacing facing)
    {
        ...
        {
            if (CulinaeRegistry.SPICE_BOTTLE.hasFluid(heldThing))
            {
                FluidStack fluid = CulinaeRegistry.SPICE_BOTTLE.getFluidHandler(heldThing).drain(Integer.MAX_VALUE, false);
                if (fluid == null || fluid.amount < 100)
                {
                    return false;
                }
            }
            Spice spice = CulinaeRegistry.SPICE_BOTTLE.getSpice(heldThing);
            if (spice != null)
            {
                if (!player.isCreative())
                {
                    CulinaeRegistry.SPICE_BOTTLE.consume(heldThing, 1);
                }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        for (NBTBase tag : items)
        {
            if (tag instanceof NBTTagCompound)
            {
                ...
                {
                    this.ingredientsForRendering.put(CulinaePersistenceCenter.deserializeIngredient(tagCompound.getCompoundTag("ingredient")), new ItemStack(tagCompound.getCompoundTag("item")));
                }
            }
        }
    }
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        ...
        for (Entry<Ingredient, ItemStack> entry : this.ingredientsForRendering.entrySet())
        {
            ...
            tagEntry.setTag("ingredient", CulinaePersistenceCenter.serialize(entry.getKey()));
        ...
        if (data.hasKey("donenesses", Constants.NBT.TAG_INT_ARRAY))
        {
            ...
            if (donenesses.length == ingredientsForRendering.size())
            {
                ...
                for (Entry<Ingredient, ItemStack> entry : ingredientsForRendering.entrySet())
                {
                    ...
                    if (entry.getValue().getItem() == CulinaeRegistry.INGREDIENT)
                    {
                        entry.getValue().getTagCompound().setInteger(CulinaeSharedSecrets.KEY_DONENESS, donenesses[i]);
                    }
    ...
    public Optional<ItemStack> serve()
    {
        ...
        ItemStack stack = new ItemStack(CulinaeRegistry.DISH);
