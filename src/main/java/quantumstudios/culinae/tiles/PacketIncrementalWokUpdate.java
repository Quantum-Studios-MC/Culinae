...
public class PacketIncrementalWokUpdate implements PacketMod
{
    ...
    public void writeDataTo(ByteBuf buffer)
    {
        ...
        if (ingredient != null)
        {
            ByteBufUtils.writeTag(buffer, CulinaePersistenceCenter.serialize(ingredient));
        }
    }
    ...
    public void readDataFrom(ByteBuf buffer)
    {
        ...
        if (!diff.isEmpty())
        {
            this.ingredient = CulinaePersistenceCenter.deserializeIngredient(ByteBufUtils.readTag(buffer));
        }
    }
