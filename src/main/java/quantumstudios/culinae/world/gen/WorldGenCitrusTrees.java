...
import quantumstudios.culinae.world.feature.WorldFeatureCitrusGenusTree;
...
public class WorldGenCitrusTrees
{
    ...
    public void decorateEvent(Decorate event)
    {
        ...
        if (event.getType() == Decorate.EventType.TREE && Arrays.binarySearch(CulinaeConfig.WORLD_GEN.fruitTreesGenDimensions, world.provider.getDimension()) >= 0 && event.getRand().nextInt(200) < CulinaeConfig.WORLD_GEN.fruitTreesGenRate)
        {
            ...
            if (pos != null && pos.getY() < 100 && CulinaeRegistry.SAPLING.canPlaceBlockAt(world, pos))
            {
