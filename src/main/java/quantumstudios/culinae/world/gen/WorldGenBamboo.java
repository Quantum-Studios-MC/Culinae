...
import quantumstudios.culinae.world.feature.WorldFeatureBamboo;
...
public class WorldGenBamboo
{
    ...
    public void decorateEvent(Decorate event)
    {
        ...
        if (event.getType() == Decorate.EventType.TREE && Arrays.binarySearch(CulinaeConfig.WORLD_GEN.bamboosGenDimensions, worldIn.provider.getDimension()) >= 0)
        {
            ...
            if (biome.getBaseHeight() > 0.4F || biome.isSnowyBiome() || biome.getRainfall() < 0.5F || biome instanceof BiomeOcean || biome instanceof BiomeSwamp || rand.nextInt(200) >= CulinaeConfig.WORLD_GEN.bamboosGenRate || rand.nextDouble() > biome.getDefaultTemperature())
            {
                return;
            }
            ...
            while (--count != 0)
            {
                ...
                else if (CulinaeRegistry.BAMBOO_PLANT.canPlaceBlockAt(worldIn, pos))
                {
                    worldIn.setBlockState(pos, CulinaeRegistry.BAMBOO_PLANT.getDefaultState());
                }
            }
