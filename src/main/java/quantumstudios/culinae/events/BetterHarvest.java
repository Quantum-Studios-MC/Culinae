...
public class BetterHarvest
{
    ...
    public void onRightClick(PlayerInteractEvent.RightClickBlock event)
    {
        ...
        // logic as of the &&, ||, etc.
        // Do not rewrite to |=.
        isCrop = isCrop || block instanceof BlockCulinaeCrops && ((BlockCulinaeCrops) block).isMaxAge(state, world, pos);
        if (isCrop)
        {
            if (Arrays.asList(CulinaeConfig.GENERAL.betterHarvestBlacklist).contains(block.getRegistryName().toString()))
            {
                return;
            }
            if (!world.isRemote)
            {
                ...
                for (ItemStack dropCandidate : drops) {
                    ...
                    if (block instanceof BlockCulinaeCrops && ItemDefinition.of(drop).equals(((BlockCulinaeCrops) block).getSeed()))
                    {
                        seed = drop;
                        break;
                    }
    ...
    private static void plant(World world, BlockPos pos, EntityPlayer player, ItemStack stack)
    {
        ...
        {
            state = CulinaeRegistry.CROPS.getVariants().get(stack.getMetadata()).getValue().getBlock().getDefaultState();
        }
        else
        {
